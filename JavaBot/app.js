...
const cmEditor = CodeMirror(document.getElementById("code-editor-container"), {
  ...
  extraKeys: {
    ...
    "Ctrl-Space": cm => cm.showHint({ hint: javaHint, completeSingle: false }),
  },
});

// ──────────────────────────────────────────
//  JAVA AUTOCOMPLETE  (identifiers only)
// ──────────────────────────────────────────

// Pure language keywords to EXCLUDE from suggestions
const JAVA_LANG_KEYWORDS = new Set([
...
function javaHint(cm) {
  ...
  const matches = anyResult.list.filter(w =>
    ...
    !JAVA_LANG_KEYWORDS.has(w) &&  // not a language keyword
...
cmEditor.on("keyup", (cm, e) => {
  ...
  _hintTimer = setTimeout(() => {
    cm.showHint({ hint: javaHint, completeSingle: false });
  }, 300);
});
...
async function generateQuestion(topic, level) {
  ...
  const levelGuide = {
    beginner: "Focus on a single basic concept. Simple class structure, minimal code required.",
    intermediate: "Combine 2-3 OOP concepts. Moderate complexity.",
    advanced: "Require solid OOP design. Multiple classes, interfaces, or design patterns.",
  }[level];

  const systemPrompt = `You are an expert Java OOP instructor generating coding exercises for a learning platform.
You MUST return a single valid JSON object and absolutely nothing else — no markdown, no explanation, no code fences.`;

  const userPrompt = `Generate a ${level}-level Java OOP coding exercise for the topic: "${topic}".
...
Return this exact JSON structure:
{
  ...
  "starter": "Java starter code as a single JSON string. Use \\n for every line break and \\t for indentation. Must have the class definition and main method where relevant."
}

Rules:
- starter MUST use \\n for newlines and \\t for indentation — do NOT embed actual newlines
- starter code MUST include the class definition and main method where relevant
- body should list exactly what fields/methods to create
...
- Return ONLY the JSON object`;
  ...
  console.log("[JavaBot] raw starter:", JSON.stringify(parsed.starter));
...
// ──────────────────────────────────────────
//  JAVA CODE FORMATTER (flat-line fallback)
// ──────────────────────────────────────────
...
function renderQuestion(q) {
  ...
  if (raw.includes("\n")) {
    ...
    starterCode = formatJavaCode(raw);
  }
  console.log("[JavaBot] final starterCode lines:", starterCode.split("\n").length);
...
async function evaluateCode(code) {
  ...
  const systemPrompt = `You are JavaBot, an expert Java OOP tutor. Review student code submissions and give structured, encouraging feedback.
...
TIP:
[One actionable OOP tip]

Be specific, encouraging, and concise.`;

  const userPrompt = `OOP Topic: ${q.topic}
